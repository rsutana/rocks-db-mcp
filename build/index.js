...
// --- Common Tool Logic ---
async function handleQuery(args, isWrite) {
...
// --- Tools ---
server.registerTool("execute_read", {
    ...
    if (!args.sql.trim().toLowerCase().startsWith("select")) {
        return { content: [{ type: "text", text: "Error: execute_read only allows SELECT queries." }], isError: true };
    }
    return handleQuery(args, false);
});
server.registerTool("execute_write", {
    ...
    app.post("/tool/:name", async (req, res) => {
        ...
        try {
            ...
            switch (name) {
                case "execute_read":
                    if (!args.sql?.trim().toLowerCase().startsWith("select")) {
                        result = { content: [{ type: "text", text: "Error: execute_read only allows SELECT queries." }], isError: true };
                    }
                    else {
